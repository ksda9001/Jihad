...
function initializePageContent() {
    ...
    // 设置页面标题
    document.title = currentVideoTitle + ' - 星空影城播放器';
...
function addPlayerControls() {
    ...
    //TODO 判断当前视频是否有上一集下一集
    const controlBar = dp.container.querySelector('.dplayer-controller');
    const playBtn = dp.container.querySelector('.dplayer-play-icon');
    ...
    if (controlBar && playBtn && !document.querySelector('.dplayer-prev')) {
        ...
        // 插入到播放按钮左右
        playBtn.parentNode.insertBefore(prevBtn, playBtn);           // 左侧插入上一集
        playBtn.parentNode.insertBefore(nextBtn, playBtn.nextSibling); // 右侧插入下一集
    }
}
